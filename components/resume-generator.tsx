...
export function ResumeGenerator({ initialResume }: { initialResume: ResumeContent | null }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {resume && (
        <article className="space-y-4 rounded-xl border border-black/10 p-6 dark:border-white/10">
          <p>{resume.summary}</p>
          <div>
            <h3 className="font-medium">Skills</h3>
            <p className="text-sm text-black/70 dark:text-white/70">{resume.skills.join(" · ")}</p>
          </div>
          <div>
            ...
            <ul className="space-y-2 text-sm">
              {resume.experience.map((exp, i) => (
                <li key={i}>
                  ...
                  <p className="text-black/70 dark:text-white/70">{exp.description}</p>
                </li>
              ))}
            </ul>
          </div>
          <div>
            ...
            <p className="text-sm text-black/70 dark:text-white/70">{resume.education}</p>
          </div>
        </article>
      )}
    </div>
