import { redirect } from "next/navigation";
import { auth } from "@/lib/auth";
import { AssistantsDirectory } from "@/components/assistants-directory";

export default async function MentorAssistantsPage() {
  const session = await auth();
  if (!session?.user) redirect("/login?callbackUrl=/mentor/dashboard/assistants");

  return (
    <main className="mx-auto max-w-3xl space-y-6 px-4 py-10">
      <div>
        <h1 className="font-display text-3xl font-bold">AI Assistants</h1>
        <p className="text-black/60 dark:text-white/60">
          Nineteen role-specific experts, each scoped to their lane.
        </p>
      </div>
      <AssistantsDirectory basePath="/mentor/dashboard/assistants" />
    </main>
  );
}
